...
struct JsonList
{
	...
	JsonList() {}
	JsonList(JsonTag tag, JsonValue node, char *key) : tag(tag), node(node), key(key) {}
...
JsonParseStatus json_parse(char *str, char **endptr, JsonValue *value, JsonAllocator &allocator)
{
	...
	int top = -1;
	bool separator = true, in_comment = false;
	while (*str)
	{
		JsonValue o;
		...
		*endptr = str++;
		...
		if (in_comment) {
			if (**endptr == '*' && *++str != '/') in_comment = false;
			continue;
		}
		...
		switch (**endptr)
		{
			...
			case ',':
				...
				continue;
			case '/':
				if (*str++ != '*') return JSON_PARSE_UNEXPECTED_CHARACTER;
				in_comment = true;
				continue;
			default:
