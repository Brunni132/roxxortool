...
struct RemappingPluginContext {
	unsigned strokeId;
	InterceptionKeyStroke& systemStroke;
	bool eaten;

	RemappingPluginContext(unsigned strokeId, InterceptionKeyStroke& destStroke)
		: strokeId(strokeId), systemStroke(destStroke), eaten(false) {}
	bool eatKey() {
		return eaten = true;
	}
	bool isPressingKey() const {
		return !(systemStroke.state & 1);
	}
	bool isDepressingKey() const {
		return (systemStroke.state & 1) != 0;
	}
};
